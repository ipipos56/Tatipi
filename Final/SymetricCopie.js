...
function bred(_buf)
{
	...
	si2=parseInt(_s/2);
	ti=0.15;
	integral_image=[19200];
	sumi=0;
	counti=0;
	index=0;
	xi1=0,yi1=0,xi2=0,yi2=0;
	for(ii=0;ii<width;ii++)
	{
		...
		for(ji=0;ji<height;ji++)
		{
			...
			sumi+=_buf[index];
			if(ii==0)
				integral_image[index]=sumi;
			else
				integral_image[index]=integral_image[index-1]+sumi;
		}
	}
	...
	for(ii=0;ii<width;ii++)
	{
		for(ji=0;ji<height;ji++)
		{
			...
			if(yi2>=height)
				yi2=height-1;
			counti=(xi2-xi1)*(yi2-yi1);
			sumi=integral_image[yi2*width+xi2]-integral_image[ti1*width+xi2];
			integral_image[yi2]
		}
	}
}
...
function grayScale(sPic){
	...
	for(var i = 0; i < total_height; i++)
		for(var j = 0; j < total_width; j++){
			...
			p = r * 0 + g * 0.413 + b * 0.587; // компонента Y из YUV
			bufPic[x] = p; 
		}
	return bufPic;
}
...
function getData(num)
{
	...
	if(raw.length==0)
	{
		brick.display().addLabel("Camera caput",0,0);
		brick.display().redraw();
		script.wait(10000);
		while(1);
	}
