...
function getData(num)
{
		...
		for (i = 0; i < height; ++i)
		{
			...
			for (j = 0; j < width; ++j)
			{
				...
				if(image[i][j] == 1)
					...
					raw[i * width + j] = 16777215;
			}
		}
		brick.display().show(raw, width, height, "rgb32");
		for (i = 0; i < height; ++i)
		{
			for (j = 0; j < 4; j++)
			{
	...
	else
	{
		camerabroke = true;
		print("Camera broke");
		brick.display().addLabel("Camera broke",1,1);
		brick.display().redraw();
		script.wait(5000);
	}
...
function getARTagValue(number)
{
	...
	if(!camerabroke)
	{
		...
		if(bortnum == 0)
		{
			...
			xdec = abs((iA + iD) / 2 - (iB + iC) / 2); 
			ydec = abs((jA + jB) / 2 - (jD + jC) / 2);
			
			print(xdec +  " " + ydec);
			
			if(1)//abs(xdec - ydec)<30 && xdec < 100 && ydec < 100 && xdec > 30 && ydec > 30)
			{
